...
namespace Airwave
{
	...
	static uint32_t GetShaderDataTypeSize(const ShaderDataType &type)
	{
		...
		AIRWAVE_ASSERT(false, "Unknown ShaderDataType")
		return -1;
	}
	...
	static uint32_t GetShaderTypeDataCount(const ShaderDataType &type)
	{
		...
		AIRWAVE_ASSERT(false, "Unknown Shader Data Type");
		return -1;
	}
	...
	{
	public:
		BufferElement(const ShaderDataType &type, const std::string &name, bool isNormalized = false) : m_Type(type),
																										m_Name(name),
